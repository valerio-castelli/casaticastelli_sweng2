...
public boolean requestTaxi(passengerID, passengerGPS){
	// We create a new request and write it into the database 
	Request request = new Request(passengerID, passengerGPS);
	dataAccessUtilities.writeToDB(request);
	/* We compute the zone of the source */
	Zone z = locationManagement.getZone(passengerGPS);
	...
	if (taxiManagement.existsAvailableTaxiDriver(zone)){
		...
		int ETA = taxiManagement.getETA(td, passengerGPS);
...
public int getETA(passengerGPS){
	return mappingService.computeETA(passengerGPS, td.getLocation());
}
...
The fourth algorithm we will focus on is the one that manages the transition of a taxi from a zone to another. The main algorithm is described by the sendGPS(taxiId,GPS) method of the TaxiManagement component.

\begin{lstlisting}
public class TaxiManagement{
	
	public void sendGPS(taxiId,GPS){
		Taxi taxi = getTaxiById(taxiDriverId);
		TaxiStatus status = checkStatus(taxiDriverId);
		/* The zone change has an impact only if the taxi is available */
		if (status==TaxiStatusAvailable){
			if (locationManagement.isLocationInsideCity(GPS)){
				/* If it's inside the city, it should change zone */
				Zone newZone = locationManagement.getZone(GPS);
