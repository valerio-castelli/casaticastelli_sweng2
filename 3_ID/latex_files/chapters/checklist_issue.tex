...
\begin{enumerate}
	...
	\item \checklist{36} The invocation of a method via java at \codeline{1905} and \codeline{1907} reflection returns an \code{Object} object which is not stored nor used. This may be coherent with the expected behavior of the setProperty(String, String) method which is found via reflection, but it's worth checking. 
	\item \checklist{36} Method \code{setMethodAction.run()} at \codeline{1933} could potentially return an \code{Object} object which is not store nor used. In practice, s\code{setMethodAction.run()} always return null, so this is not particularly significant; however, it highlights a bad design decision, as \code{setMethodAction.run()} should probably be a void method instead. 
	...
	\item \checklist{52} The \code{catch(Exception ex)} clause at \codeline{1914} is very generic. This could be appropriate for the context as it may be difficult to predict all possible exceptions of the setProperty(String, String) method, supposing that this method can be developed in many different ways depending on the specific ManagedConnectionFactory implementation (in fact, it's not explicitly defined by the interface); however, it is still useful to highlight this, as maybe it could be further refined depending on the exact nature of the object we are expecting to deal with. 
