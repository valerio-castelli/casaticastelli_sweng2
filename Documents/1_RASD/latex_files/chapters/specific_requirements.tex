...
\begin{figure}[H]
\centering
\makebox[\columnwidth]{\includegraphics[width=300pt,keepaspectratio]{images/guest_home.png}}
\end{figure}
...
\begin{figure}[H]
\centering
\makebox[\columnwidth]{\includegraphics[width=300pt,keepaspectratio]{images/guest_login.png}}
\end{figure}
...
\subsubsection*{Sketch 5}
The following sketch shows a possible implementation of the personal webpage of a logged-in passenger in the web application. From this page, a logged-in passenger can request a taxi, place a reservation for a taxi, see the reservations he has already placed and access his personal settings.
\begin{figure}[H]
\centering
\makebox[\columnwidth]{\includegraphics[width=300pt,keepaspectratio]{images/user_home.png}}
\end{figure}
...
\begin{figure}[H]
\centering
\makebox[\columnwidth]{\includegraphics[width=300pt,keepaspectratio]{images/user_reservation.png}}
\end{figure}


\subsubsection*{Sketch 7}
...
\begin{figure}[H]
\centering
\makebox[\columnwidth]{\includegraphics[width=150pt,keepaspectratio]{images/taxi_accepted_ride.png}}
\end{figure}


\subsubsection*{Sketch 8}
The following sketch shows a possible implementation of the main screen of the taxi driver mobile application. In this screen, a taxi driver can see his current status, check the zone he's in and the number of taxis ahead of him in the queue. He can also change his status to unavailable.
\begin{figure}[H]
\centering
\makebox[\columnwidth]{\includegraphics[width=150pt,keepaspectratio]{images/taxi_available.png}}
\end{figure}


\subsubsection*{Sketch 9}
The following sketch shows a possible implementation of the login screen of the taxi driver mobile application.
\begin{figure}[H]
\centering
\makebox[\columnwidth]{\includegraphics[width=150pt,keepaspectratio]{images/taxi_login.png}}
\end{figure}
...
\begin{figure}[H]
\centering
\makebox[\columnwidth]{\includegraphics[width=150pt,keepaspectratio]{images/taxi_meeting.png}}
\end{figure}


\subsubsection*{Sketch 11}
...
\begin{figure}[H]
\centering
\makebox[\columnwidth]{\includegraphics[width=150pt,keepaspectratio]{images/taxi_new_ride.png}}
\end{figure}
...
\begin{figure}[H]
\centering
\makebox[\columnwidth]{\includegraphics[width=150pt,keepaspectratio]{images/taxi_unavailable.png}}
\end{figure}
...
\item The system must provide a way for the operator to confirm that the initial data entry operation has been completed. 
\item The system will not allow any operation to take place until the data entry phase has been marked as completed.
\item The system must provide a way for the operator to manage the list of third party services and plugins that have access to the APIs, with the ability to grant and revoke permissions.
\end{itemize}
...
\item The system must be able to distinguish the different states in which a taxi could be: available, unavailable, currently on a ride, out of the city, pending confirmation.
\item If a taxi driver which has marked himself as available changes zone while he's not on a ride, the system should move his taxi code in the queue of the new zone.
...
\item If a taxi whose status is set as ``out of the city'' enters the city and the taxi was available before leaving the city and the driver has not marked himself as unavailable, the system should assign it to the queue of the zone in which it has entered.
\item If a taxi whose status is set as ``out of the city'' enters the city and the driver had marked himself as unavailable while he was out, the system should set its status as ``unavailable'' and move its code to the unavailability list.
...
\item If a taxi leaves the city while it's unavailable, the system should prevent its driver from signaling he's available until he is again inside the perimeter of the city.
...
\item The system should provide an appropriate interface to let a driver accept or refuse a ride request.
...
\item If it's not possible to establish an internet connection to the central system, the mobile application of the taxi driver should notify him that it's unable to operate correctly.
...
\item If the destination of the ride is known (as in the case of reservations), a taxi driver should be able to notify that he's terminated a ride only after the system has checked that the destination point has actually been reached within a given level of precision. If the destination of the ride is unknown, this check is not required.
...
In order to keep maintenance costs low, the system should offer a level of reliability such that no key component of the system may fail more than once a year.


\subsection{Availability}
The system is required to have an availability of 99.99\%. This means that it's considered acceptable to have a downtime of up to 1 hour every year due to malfunctioning of the system. This implies that, at the architectural level, the system has to include a certain amount of replication of its hardware structures in order to overcome potential failures.
...
Moreover, the central infrastructure of the system must implement proper mechanisms to be resistant to external attacks.

In particular, it should be robust with respect to DDoS, SQL injection and other common attack vectors; furthermore, only the connections necessary for the correct operations of the services should be allowed, while all the others must be blocked by a physical firewall.
For what concerns security of users' (taxi drivers and passengers) data, passwords have to be encrypted.

The disk drives of the servers that are going to run the system should be encrypted as well, in order to neutralize physical attacks to the infrastructure.

Adequate protection against fire, floods and other disasters should be taken into account.


\subsection{Portability}
The system should be designed to be easily portable. In particular, the mobile side of the system should be coded using a programming language and a set of frameworks that can run on the major mobile platforms (iOS, Android, Windows Phone) with the option of being easily portable on new ones, in order to anticipate future market changes. No platform-dependent frameworks should be used while coding the mobile applications.
